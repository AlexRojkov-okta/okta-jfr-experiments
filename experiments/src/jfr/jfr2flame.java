    ...
    public void convert(final FlameGraph fg, final boolean threads, final boolean total,
        ...
        agg.forEach(new EventAggregator.Visitor() {
            ...
            public void visit(Event event, long value) {
                ...
                Set<String> focusThreads = new HashSet<>();
                focusThreads.add("localhost-startStop-1");
                focusThreads.add("OktaQuartzScheduler_QuartzSchedulerThread");

                if (!focusThreads.contains(threadName)) {
                    return;
                }
                ...
                if (stackTrace != null) {
                    ...
                    for (int i = 0; i < methods.length; i++) {
                        ...
                        boolean isOktaMethod = true || (methodName.contains("okta") || methodName.contains("saasure"));

                        if (isOktaMethod) {
                            lastOktaIdx = idx;
                        }

                        isOkta = isOkta || isOktaMethod;
                    }

                    if (isOkta) {
                        LinkedList<String> localTraces = new LinkedList<>();
                        localTraces.add(trace[0]);
                        for (int i = lastOktaIdx; i < trace.length; i++) {
                            String method = trace[i];

                            localTraces.add(method);
                        }

/*
                        for (String method : trace) {
                            if (method.contains("/saasure/")
                                    || method.contains("/okta/")
                                    || method.contains("/io/")
                                    || method.contains("ibernate/")
...
*/
                        //fg.addSample(trace, scale ? (long) (value * ticksToNanos) : value); //original
                        fg.addSample(localTraces.toArray(new String[0]), scale ? (long) (value * ticksToNanos) : value);
                    }
                }
            }
        });
